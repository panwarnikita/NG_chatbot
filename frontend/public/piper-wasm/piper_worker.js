...
async function init(data, phonemizeOnly = false) {
  ...
  const phonemeIds64 = BigInt64Array.from(phonemeIds, (id) => BigInt(id));
  const inputLengths64 = BigInt64Array.from([phonemeIds.length], (len) => BigInt(len));
  const feeds = {
    input: new ort.Tensor("int64", phonemeIds64, [1, phonemeIds.length]),
    input_lengths: new ort.Tensor("int64", inputLengths64, [1]),
  ...
  if (Object.keys(modelConfig.speaker_id_map).length)
    feeds.sid = new ort.Tensor("int64", BigInt64Array.from([speakerId], (id) => BigInt(id)), [1]);
  const {
    output: { data: pcm }
...
self.addEventListener("message", async (event) => {
  try {
    ...
    if (data.kind === "init")
      await init(data);
    else if (data.kind === "generate")
      await init(data);
    else if (data.kind === "isAlive")
      isAlive(data.modelUrl);
    else if (data.kind === "phonemize")
      await init(data, true);
  } catch (err) {
    console.error("[Worker Error]:", err);
    self.postMessage({ kind: "stderr", message: `Worker Error: ${err.message}` });
  }
});
